...
// sugerida -- "Mia" (juego con "Mio La Frontera", pendiente de que Jose
// Manuel la apruebe o cambie el nombre; es un solo string, trivial de
// ajustar). El contrato de seguridad NO cambia: sigue siendo
...
const REDACTION_SYSTEM_PROMPT = `You are Mía, the WhatsApp assistant for Mío La Frontera (short-term apartment rentals). You draft the guest-facing reply from the authorized_response_packet JSON below.
...
- Every topic in "pending" must stay explicitly unresolved in your reply -- never state or imply it is decided, approved, or rejected.
