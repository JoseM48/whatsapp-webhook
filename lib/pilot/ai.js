...
const interpretationSchema = {
  ...
  properties: {
    ...
    // P0-2 (2026-09-24): SENAL PROPIA PARA CANCELAR.
    // Sin ella, el contrato no tenia donde poner "el huesped quiere
    // cancelar", y la unica casilla parecida era exception_request. Medido
    // en produccion (M0-20260924-FD2FAD8E): CANCELAR sobre una pre-reserva
    // salio como exception_request=true -> ESCALAR A HUMANO, con la
    // pre-reserva viva y el bloqueo sin soltar. Cancelar no es negociar.
  ...
  required: [
    ...
    'requests_human', 'exception_request', 'cancellation_request', 'payment_reported', 'uncertainty', 'needs_clarification', 'missing_fields'
  ]
};
...
function deterministicInterpret(text, { today, context = {} } = {}) {
  ...
  const interpretation = {
    ...
    cancellation_request: false, payment_reported: false,
    ambiguity: [], unmapped_meaning: null, duration_kind: 'none', guest_composition: null,
    visitors_question: false, references: [],
...
function reconcileInterpretation(text, modelInterpretation = {}, { today, context = {} } = {}) {
  ...
  const reconciled = {
    ...
    cancellation_request: modelInterpretation.cancellation_request === true,
    payment_reported: modelInterpretation.payment_reported === true,
    // Experimento (2026-09-24): la ruta legacy no produce estas senales;
    // viajan vacias para que el contrato con el PMS sea el mismo en ambas.
    ambiguity: [], unmapped_meaning: null, duration_kind: 'none', guest_composition: null,
    visitors_question: false, references: []
  };
