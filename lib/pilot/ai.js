...
// deben quitarse antes de enviar el schema -- interpretationSchema y las
// demas siguen exportando estas claves tal cual (documentan el contrato
// real de datos), esta funcion solo filtra lo que viaja a la API.
const UNSUPPORTED_SCHEMA_KEYS = new Set(['minLength', 'maxLength', 'minimum', 'maximum', 'multipleOf']);
...
class PilotAi {
  ...
  constructor({ client, apiKey, model = 'claude-sonnet-5', safetySalt = '' } = {}) {
    this.model = model;
  ...
  async interpret({ text, phone, today, context = {} }) {
    ...
    try {
      ...
      console.error('[pilot-ai] interpretation_fallback', { ...safeDependencyError(error), message: String(error?.message || '').slice(0, 300) });
      return {
  ...
  async redact({ packet, phone }) {
    ...
    try {
      const candidate = await this.structured({
        ...
        system: 'You draft the guest-facing WhatsApp reply from the authorized_response_packet JSON below. '
          + 'You may freely change greeting, order, connectors, tone and brevity. '
          + 'You must NEVER change facts, invent information, or decide availability, pricing, exceptions, or reservations -- '
          + 'only what is already in the packet is true. '
          + 'Every value in "numbers" and every value in "dates" must appear in your reply exactly as given, same formatting, never reformatted or translated. '
          + 'Every entry in "required_disclosures" must appear in your reply verbatim, word for word, never paraphrased. '
          + 'Every topic in "pending" must remain explicitly unresolved and unconfirmed in your reply -- never state or imply it is decided, approved, or rejected. '
          + 'Every entry in "questions_to_ask" must still be asked, in your own words, and your reply must end with a question mark if any are present. '
      ...
      return { text: candidate.text, model: this.model, latency_ms: Date.now() - startedAt };
    } catch (error) {
      return {
