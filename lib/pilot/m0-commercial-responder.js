...
function createM0CommercialResponder({ capture, ai, closedPilot, pms, logger = console,
  ...
  async function captureAndAcknowledge({ from, text, messageId, timestamp, name, referral }) {
    const startedAt = clock();
    let captureResult;
    try {
      captureResult = await capture({ from, text, messageId, timestamp, name, referral });
    } catch (error) {
