...
function createM0CommercialResponder({ capture, ai, closedPilot, pms, logger = console,
  ...
  async function captureAndAcknowledge({ from, text, messageId, timestamp, name, referral, landingRef }) {
    const startedAt = clock();
    let captureResult;
    try {
      captureResult = await capture({ from, text, messageId, timestamp, name, referral, landingRef });
    } catch (error) {
  ...
  async function processCaptured({ from, text, messageId, today, context = {}, processingClaimed = true }) {
    ...
    try {
      ...
      const completed = await closedPilot.completeCommercial({
        ...
        // Experimento "el LLM escribe" (2026-09-24): el redactor ve lo mismo
        // que vio el interprete, mas lo que el PMS decidio.
        writerInput: { guestText: text, transcript: Array.isArray(context.transcript) ? context.transcript : [],
          language: interpreted?.language || context.language_preference || null, today }
      });
      return {
        ok: true,
        completed,
