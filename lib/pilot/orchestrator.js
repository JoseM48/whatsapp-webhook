...
class PilotOrchestrator {
  ...
  async capture({ from, text, messageId, timestamp, name, referral, landingRef }) {
    ...
    return this.pms.capture({
      ...
      landing_ref: landingRef || undefined,
      // Incremento 062: este origen es el que activa la inscripcion en
      // supervised_pilot_lead_cohort del lado de PMS (whatsapp-inbound.
      // service.js) -- con el origen anterior ('whatsapp_oficial_pilot_
      // controlado') esa inscripcion nunca se ejecutaba para un numero
      // nuevo, confirmado con datos reales de produccion (los 2 leads ya
      // inscritos muestran este origen en todas sus interacciones).
      external_message_id: messageId, origen: 'whatsapp_oficial_controlled_ingress',
