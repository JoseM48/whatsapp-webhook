...
const interpretationSchemaV2 = {
  ...
  properties: {
    ...
    references: {
      ...
      items: {
        ...
        properties: {
          ...
          // Experimento "el LLM escribe" (2026-09-24): el DESENLACE de la
          // referencia viaja al PMS. "Me quedo con el 9999" era una referencia
          // fallida que se perdia en la proyeccion y acababa como busqueda nueva
          // (auditoria, B7). Con `out_of_scope` el motor puede responder a eso.
          status: { type: 'string', enum: ['ok', 'unresolved', 'ambiguous', 'out_of_scope'] }
        },
        required: ['kind', 'raw', 'hint_type', 'hint_value', 'resolved_code', 'status']
      }
    },
    ...
    // Tramo de pago (2026-09-25): "ya pague", "transferi", "te mando el
    // comprobante", "PAGO ENVIADO". El PMS decide segun el estado del caso.
    payment_reported: { type: 'boolean' },

    // Experimento (2026-09-24): "¿puedo recibir 10 visitantes?" NO son 10
    // huespedes. Sin esta senal, el numero acababa guardado como `guests` del
    // caso y disparaba las preguntas de fechas (auditoria, B8).
    visitors_question: { type: 'boolean' },
  ...
  required: [
    ...
    'requests_human', 'exception_request', 'cancellation_request', 'payment_reported', 'visitors_question', 'suggests_confirmation'
  ]
};
...
function projectToLegacyInterpretation(v2) {
  ...
  // Visitantes no son huespedes: no se declara `guests` como dato aportado.
  if (guests.total && !v2.visitors_question) provided.push('guests');
  ...
  return {
    ...
    nights: duration.nights ?? null, guests: v2.visitors_question ? null : (guests.total ?? null),
    ...
    payment_reported: Boolean(v2.payment_reported),
    // Experimento "el LLM escribe" (2026-09-24): lo que antes se descartaba
    // en esta proyeccion ahora viaja al PMS, que lo acepta con defaults.
    ambiguity: (v2.ambiguity || []).map((a) => ({ field: a.field, reason: a.reason, candidates: a.candidates || [] })),
    unmapped_meaning: v2.unmapped_meaning ?? null,
    duration_kind: duration.kind || 'none',
    guest_composition: (guests.adults ?? guests.children ?? guests.infants) != null
      ? { adults: guests.adults ?? null, children: guests.children ?? null, infants: guests.infants ?? null } : null,
    visitors_question: Boolean(v2.visitors_question),
    references: (v2.references || []).map((r) => ({ raw: r.raw, hint_type: r.hint_type, hint_value: r.hint_value ?? null,
      resolved_code: r.resolved_code ?? null, status: r.status || (r.resolved_code ? 'ok' : 'unresolved') })),
