...
async function runSupervisedReservationConfirmationRelay({
  ...
  if (!['v1', 'diagnostic'].includes(gateVersion)) return { executed: false, reason: 'gate_disabled' };
  ...
  if (gateVersion === 'diagnostic') {
    logger.info('[m0-supervised-outbound] eligibility', summary?.eligibility || null);
    return { executed: false, reason: 'diagnostic_only', eligibility: summary?.eligibility || null };
  }
