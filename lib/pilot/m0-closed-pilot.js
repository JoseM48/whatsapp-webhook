...
function createM0ClosedPilotDispatcher({ config, pms, sendText, sendTemplate, sendFlow, logger = console }) {
  ...
  async function deliver(outbox) {
    ...
    // have used before this feature existed. While the Flow is still a draft
    // (mode:'draft'), it is restricted to the authorized test phones so a real
    // guest never gets pointed at an unpublished form.
    const flowTestGateOpen = config.flow?.mode !== 'draft' || isAllowlisted(recipient, config.flow?.testAllowlist || []);
      ...
      providerReference = claim.recipient_kind === 'internal'
        ...
        : useFlow
          ? await sendFlow(recipient, { flowId: config.flow.flowId, flowToken: `${claim.outbox_id}`,
            firstScreen: config.flow.firstScreen, ctaText: 'Continuar', bodyText: claim.message_text,
            mode: config.flow.mode || undefined })
