...
function createM0ClosedPilotDispatcher({ config, pms, sendText, sendTemplate, sendFlow, sendPhoto, logger = console,
  ...
  async function deliver(outbox, { packet = null } = {}) {
    ...
    // TEMP DEBUG 2026-09-04 (verificacion de config efectiva tras la
    // apertura a huespedes reales, autorizacion Jose Manuel) -- se retira
    // apenas se confirme, no es codigo permanente. Opcional a proposito:
    // no todos los stubs de `logger` en pruebas implementan .info.
    logger?.info?.('[np-open-debug]', {
      naturalPresentationEnabled, restrict: config.naturalPresentationRestrictToTestPhones,
      raw_env_restrict: process.env.M0_NATURAL_PRESENTATION_RESTRICT_TO_TEST_PHONES
    });
