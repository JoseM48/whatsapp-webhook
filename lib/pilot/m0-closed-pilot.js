...
const { resolveWrittenReply } = require('./llm/writer');
...
function createM0ClosedPilotDispatcher({ config, pms, sendText, sendTemplate, sendFlow, sendPhoto, logger = console,
  ...
  redactionAi = null,
  // EXPERIMENTO "el LLM escribe" (2026-09-24): proveedor con el que el MISMO
  // modelo que entendio el turno escribe la respuesta. Se pasa como funcion
  // para no depender del orden de construccion en index.js.
  writerProvider = null, writerTimeoutMs = 20000 }) {
  ...
  async function deliver(outbox, { packet = null, writerInput = null } = {}) {
    ...
    // EXPERIMENTO "el LLM escribe" (2026-09-24). Con un paquete v2 (PMS
    // nuevo) y un proveedor de redaccion, el texto al huesped lo escribe el
    // modelo con la conversacion delante, y el validador del PMS lo aprueba.
    // Con paquete v1 (PMS anterior) se conserva el camino de siempre
    // (parafrasis por Claude), asi que el webhook nuevo funciona contra
    // cualquiera de los dos PMS: es lo que permite desplegar en cualquier
    // orden sin estados mixtos peligrosos.
    const provider = typeof writerProvider === 'function' ? writerProvider() : writerProvider;
    const paqueteV2 = Number(packet?.packet_version) === 2;
    let presentation = null;
    if (paqueteV2 && provider && packet && claim.recipient_kind === 'guest' && claim.message_kind !== 'photos' && !useGuestTemplate) {
      presentation = await resolveWrittenReply({
        packet, provider, pms, guestText: writerInput?.guestText || '',
        transcript: writerInput?.transcript || [], interpretation: writerInput?.interpretation || null,
        language: writerInput?.language || null, today: writerInput?.today || null,
        timeoutMs: writerTimeoutMs, logger
      });
      guestText = presentation.text ?? claim.message_text;
      logger.info('[m0-closed] llm_written_reply', {
        outbox_id: outbox.id, presentation_source: presentation.presentation_source,
        attempts: presentation.attempts, latency_ms: presentation.latency_ms,
    ...
    if (!paqueteV2 && naturalPresentationEnabled && naturalPresentationTestGateOpen && redactionAi && packet && claim.recipient_kind === 'guest' && claim.message_kind !== 'photos' && !useGuestTemplate) {
    ...
    try {
      ...
      // El texto que DE VERDAD salio queda en el outbox: es lo que la ventana
      // de conversacion le mostrara a Cami en el siguiente turno.
      await pms.completeClosedPilotOutbound({ outbox_id: claim.outbox_id, status: 'submitted',
        provider_reference: providerReference,
        ...(claim.recipient_kind === 'guest' && guestText && guestText !== claim.message_text ? { sent_text: guestText } : {}) });
  ...
  return {
    ...
    async completeCommercial({ externalMessageId, interpretation, ai, writerInput = null }) {
      ...
      // Experimento (2026-09-24): ademas viaja lo que el redactor necesita
      // ver -- mensaje del huesped, conversacion e interpretacion.
      return { handled: true, result, deliveries: await deliverAllSafe(result?.outboxes || [],
        { packet: result?.authorized_response_packet || null,
          writerInput: writerInput ? { ...writerInput, interpretation } : null }) };
    },
