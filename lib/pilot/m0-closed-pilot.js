...
function createM0ClosedPilotDispatcher({ config, pms, sendText, sendTemplate, sendFlow, logger = console }) {
  ...
  async function deliver(outbox) {
    ...
    // is configured and published; otherwise it degrades to the same text
    // question it would have used before this feature existed.
    const useFlow = claim.recipient_kind === 'guest' && claim.message_kind === 'flow' && flowReady;
